...
- 기본 레인 ID를 Notion 담당자 값과 맞췄습니다: `기획`, `디자인`, `개발`, `pm`.
- Notion DB의 `담당자` 값이 `기획/디자인/개발/PM`이면 기존 레인에 들어갑니다.
- 위 네 가지에 없는 담당자 값이 들어올 때만 새 레인이 추가됩니다.
- 공동 업무는 `담당자`를 `공통`으로 만들거나, Multi-select로 여러 담당자를 넣으면 됩니다.
